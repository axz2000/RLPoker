...
int main(int argc, char* argv[]) //ex: six 4     for the 4th sixI2H file
{
    initiateAllCards();
    if(initiateHR()==-1) return -1;
    
    string a = string(argv[1]); string b = string(argv[2]);
    string fname = "./"+a+"I2Hs"+b+".txt";
    string fname2 = "./"+a+"I2sEHS"+b+".txt";
    ...
    if (myReadFile.is_open())
    {
        while (!myReadFile.eof())
        {
            ...
            {
                int temp = atoi(line.substr(idx+i*2,2).c_str());
                cards[i] = temp;
            }
            if(numCards==2)
                val = EHS2(cards);
                ...
                val = EHS6(cards);
            myWriteFile << to_string(id)+":"+to_string(val).substr(0,5)+"\n";
            myWriteFile.flush();
        }
    }
    myWriteFile.close();
